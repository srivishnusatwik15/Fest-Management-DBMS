...
class Frame{
    ...
    bool second_chance;  
    int arrival_time;       // for clock replacement algorithm
    ...
    friend class  FIFOBufferManager;
...
class FIFOBufferManager: public ReplacementPolicy{
    private:
    int num_Frames;    // number of frames that can be fit in pool
    vector<Frame> fifo;  // vector to implement FIFO
    unordered_map<pair<FILE*, int>, int, PairHash> mp;   // map to identify whether a page is present in buffer or not and to store its index in FIFO vector
    queue<int> fifoQueue;  // queue to keep track of the order of pages in FIFO
    BufStats stats;
    // int next_arrival_time;  
       void evictOldestPage();
    public:
    FIFOBufferManager(int num_Frames);
    char* getPage(FILE*fp, int page_Num) override;
    ~FIFOBufferManager() override;
